...
as property>${property.columnName}<#if property_has_next>,</#if></#list>);");
    ...
    public static void upgradeTable(SQLiteDatabase db, boolean ifExists) {
	    ...
	    int count=0;
        String constraint = "";
        String sql = "SELECT COUNT(*) as count FROM sqlite_master WHERE TYPE='table' AND NAME='${entity.tableName}'";
        count=0;
        try {
	    ...
	    <#list entity.propertiesColumns as property>
	    	count=0;
	    	if(!errorTag){
		    	querySql="SELECT COUNT(*) as count FROM sqlite_master WHERE tbl_name='${entity.tableName}' AND [sql] LIKE '%''${property.columnName}''%' ";
		    	Cursor c${property.columnName} = null;
		    	try{
			    	c${property.columnName} = db.rawQuery(querySql,null);
			    	if(c${property.columnName}.moveToFirst()){
			    		count=c${property.columnName}.getInt(c${property.columnName}.getColumnIndex("count"));
			    	}
			    	if(count == 0){
						alertSql = "ALTER TABLE "+constraint +"${entity.tableName} ADD COLUMN '${property.columnName}' ${property.columnType}";
			        	db.execSQL(alertSql);
					}
		    	}catch (Exception e) {
					e.printStackTrace();
					...
					if(c${property.columnName}!=null){
						c${property.columnName}.close();
						c${property.columnName} = null;
					}
				}
